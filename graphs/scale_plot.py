#!/usr/bin/env python3
# scale_line.py — Worker vs Knitting (Scale test: 1 core, wide message range)

import re, os, json, math, argparse
import matplotlib.pyplot as plt

RUNTIME_FILES = {
    "Node.js": "../results/node_scale.json",
    "Deno":    "../results/deno_scale.json",
    "Bun":     "../results/bun_scale.json",
}

RUNTIME_COLORS = {"Node.js": "#1f77b4", "Deno": "#2ca02c", "Bun": "#d62728"}
KNIT_COLORS    = {"Node.js": "#aec7e8",  "Deno": "#98df8a", "Bun": "#ff9896"}

# capture "(10)"/"(100)"/"(1000)" and also "→ 5000"
COUNT_RE = re.compile(r"(?:→\s*|\()\s*(\d{1,6})\b")
...
def align_by_count(rows):
    ...
    # fallback: index-align (shouldn’t be needed if counts exist)
...
def main():
    ...
    ap.add_argument("--input", "-i", default="./results", help="folder with *_scale.json")
    ap.add_argument("--out", "-o", default="./charts/scale_line.png", help="output PNG path")
    args = ap.parse_args()
