import { HAS_SAB_GROW, isGrowableSharedArrayBuffer, sharedArrayBufferMaxByteLength, } from "../common/runtime.js";
...
const sharedBufferMaxByteLength = (sab) => {
    if (sab == null)
        return undefined;
...
export const resolvePayloadBufferOptions = ({ options, sab, }) => {
    ...
    const payloadMaxByteLength = toPositiveInteger(options?.payloadMaxByteLength) ??
    ...
    const payloadInitialBytes = mode === "fixed"
        ...
        : Math.min(requestedInitialBytes ?? PAYLOAD_DEFAULT_INITIAL_BYTES, payloadMaxByteLength);
