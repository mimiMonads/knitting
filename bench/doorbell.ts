 ...
 * are identical between modes. Results include throughput, completion latency,
 * and main-thread CPU from /proc when available.
 */
...
if (isMain) {
  ...
  const pool = createPool({ threads: THREADS, host })({ mixed });

  try {
    await Promise.all(inputs.slice(0, CONCURRENCY).map((value) => pool.call.mixed(value)));
    ...
    for (let rep = 0; rep < REPS; rep++) {
      ...
      for (let offset = 0; offset < inputs.length; offset += CONCURRENCY) {
        ...
        const pending = batch.map((value) =>
          pool.call.mixed(value).then((result) => {
